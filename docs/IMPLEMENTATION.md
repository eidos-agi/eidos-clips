...
- Checked MP4 export, native playback, title changes, trim sliders that export a new file, searchable recordings with thumbnails, Finder access, and the macOS share sheet. Originals remain intact. Two audio sources receive equal fixed gain with mix headroom; there is no speech-triggered ducking or echo-cancellation claim.
- A development app bundle with its own icon and bundle identity; a separate company Developer ID/notarization release script; pinned CI action revisions, Xcode 16.4, and tests that use real AVFoundation encoding/decoding on synthetic video and two audio tones.
...
CI uploads the app only after these checks succeed. `foundation-evidence.json` and `swift-tests.log` are authoritative for the actual run, not this list of intended checks. A separate `--ui-smoke` launch opens the real native window and renders recording, library, review, and compact layouts using a synthetic clip. CI retains those PNGs and `ui-smoke.json` for visual inspection. It never starts screen, mic, or camera capture. The media probe is headless and uses synthetic input.

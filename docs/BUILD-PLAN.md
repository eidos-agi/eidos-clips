...
| Capture control | Cancellable countdown/preparation, remembered input choices, mic/camera selectors, global shortcuts, sleep interruption | Compile and shortcut registration evidence | Input meters, device hot-plug, live mute/camera controls, lock and sleep matrix |
...
3. **Close reliable capture gaps.** Add separate input meters/test capture, window selection/preview, live mute/camera controls, device/display loss policy, true source-border proof and two-hour qualification. Preserve failure-safe required-input semantics.
